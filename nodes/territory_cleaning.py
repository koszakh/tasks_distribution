...
garbage_ids = mh.get_random_points_in_area(goal[0], goal[1], const.GARBAGE_DIST_OFFSET, garbage_count)
...
if not garbage_detected:
	...
	det_g_count = random.randint(1, garbage_count)
	detected_garbage = {}
	tmp_garbage_ids = copy.copy(garbage_ids)
	garbage_poses = garbage_ids
	for i in range(det_g_count):
	
		g_key = random.choice(list(tmp_garbage_ids.keys()))
		g_pos = tmp_garbage_ids[g_key]
		tmp_garbage_ids.pop(g_key)
		detected_garbage[g_key] = g_pos
